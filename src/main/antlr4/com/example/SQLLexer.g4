...
DESC        : D E S C;  // Can be Describe or Descending
...
// =====================
FLOAT_LITERAL
    : [0-9]+ '.' [0-9]* ([eE] [+-]? [0-9]+)?  // Matches 1.2, 1., 1.2e5
    | '.' [0-9]+ ([eE] [+-]? [0-9]+)?         // Matches .5, .5e2
    | [0-9]+ [eE] [+-]? [0-9]+                // Matches 1e5 (no dot, but has exponent)
    ;
...
STRING_LITERAL : '\'' ( ~'\'' | '\'\'' )* '\'';
UNICODE_STRING : [nN] '\'' ( ~'\'' | '\'\'' )* '\'';
BINARY_STRING  : [xX] '\'' [0-9a-fA-F]* '\'';
BIT_STRING     : [bB] '\'' [01]* '\'';
DOLLAR_STRING  : '$' [a-zA-Z0-9_]* '$' .*? '$' [a-zA-Z0-9_]* '$'; 
...
// Variables
BIND_PARAMETER    : '?' | ':' [a-zA-Z_][a-zA-Z0-9_]*;
